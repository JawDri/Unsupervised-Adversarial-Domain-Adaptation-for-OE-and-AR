...
def create_resnet_lower(model_name='resnet50', pretrained=False):
    """
...
def create_resnet_upper(model_name='resnet50', pretrained=False, num_classes=3):###########
    """
    ResNet options: ResNet50, ResNet101, ResNet152
    :param model_name: resnet50, resnet101, resnet152
    :param pretrained: call pretrained model
...
def resnet50(pretrained=False, is_lower=True, num_classes=3, **kwargs):###########
    model = ResNetLower(Bottleneck, [2, 2, 2, 2], **kwargs) if is_lower else ResNetUpper(num_classes=num_classes)
    if pretrained:
...
'''
def resnet101(pretrained=False, is_lower=True, num_classes=2, **kwargs):
...
def resnet152(pretrained=False, is_lower=True, num_classes=2, **kwargs):
    model = ResNetLower(Bottleneck, [1, 8, 36, 2], **kwargs) if is_lower else ResNetUpper(num_classes=num_classes)
    if pretrained:
...
class ResNetLower(AbstractModel):
    ...
    def __init__(self, block, layers):
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1, use_dropout_after_first_bottleneck=True)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, use_dropout_after_first_bottleneck=False):
        ...
        layers = []
        dropout_module = nn.Dropout1d(0.1) if use_dropout_after_first_bottleneck else None
...
class ResNetUpper(AbstractModel):
    def __init__(self, in_features=1000, num_classes=3):############
        ...
        self.relu = nn.ReLU(inplace=True)
        self.fc1 = nn.Linear(53092, 1000)#6126/53092##################
