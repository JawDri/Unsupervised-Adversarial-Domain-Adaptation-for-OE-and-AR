...
class DTATrainer(object):
    ...
    def train_one_epoch(self, epoch, dataloader, accum_iter):
        ...
        for batch_idx, ((source_inputs, source_labels), (target_inputs, target_labels)) in enumerate(tqdm_dataloader):
            source_inputs = source_inputs.view(-1,1, 32)############
            target_inputs = target_inputs.view(-1,1, 32)############
            ...
            average_meter_set.update('target_accuracy', target_predicted.eq(target_labels).sum().item(),
                                     n=batch_size)
            #average_meter_set.update('target_fscore', f1_score(target_labels.cpu(), target_predicted.cpu(), average='weighted'),
                                        # n=batch_size)#############
    ...
    def validate(self, epoch, dataloader, accum_iter):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (target_inputs, target_labels) in enumerate(tqdm_dataloader):
                ...
                average_meter_set.update('target_fscore', f1_score(target_labels.cpu(), target_predictions.cpu(), average='weighted')
                                         )##################
                

            print("\ntarget_ce_loss: {:.3f}, target_accuracy: {:.3f}, target_fscore: {:.3f}".format(
                average_meter_set['target_ce_loss'].avg, 100 * average_meter_set['target_correct'].avg,100 * average_meter_set['target_fscore'].avg,
            ))
